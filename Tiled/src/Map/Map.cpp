...
std::shared_ptr<Tile> Chunk::getTileByGlobalPosition() {
	...
	if (rect.contains(cursorOnMap->_position)) {
		int x = cursorOnMap->_position.x / Tile::side;
		int y = cursorOnMap->_position.y / Tile::side;
...
void Map::handleEvent(const sf::Event& event) {
	...
	if (ElementGUI_hovered.get() == this) {
		if (const auto* mbp = event.getIf<sf::Event::MouseButtonPressed>(); mbp && mbp->button == sf::Mouse::Button::Left)
			ElementGUI_pressed = shared_from_this();
	}

	if (!((ElementGUI_hovered.get() == this && ElementGUI_pressed == nullptr) || ElementGUI_pressed.get() == this)) return;
