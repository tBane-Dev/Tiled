...
int main() {
	...
	while (window->isOpen()) {
		...
		while (const std::optional event = window->pollEvent()) {
			...
			if (const auto* kp = event->getIf<sf::Event::KeyPressed>(); kp && kp->code == sf::Keyboard::Key::Add) {
				palette->_brushSize++; 
			}

			if (const auto* kp = event->getIf<sf::Event::KeyPressed>(); kp && kp->code == sf::Keyboard::Key::Subtract) {
				palette->_brushSize--;
			}
